...
pub fn world_coordinate_from_grid(grid_coordinate: (i32, i32)) -> (f32, f32) {
    // conversion formula: tile_size * coordinate - (tile_size * (map_width / 2))
    let x = TILE_SIZE * grid_coordinate.0 as f32 - (TILE_SIZE * (MAP_WIDTH / 2) as f32);
    let y = TILE_SIZE * grid_coordinate.1 as f32 - (TILE_SIZE * (MAP_HEIGHT / 2) as f32);
